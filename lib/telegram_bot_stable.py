...
sys.path.append(current_folder + '\\ffmpeg')
os.environ['PATH'] += ';' + current_folder + '\\ffmpeg'
import openai
import json
import requests
import telebot
...
def botactions(bot):
    ...
    def get_speaker(message):
        ...
        sent = bot.reply_to(message, f'To change speaker, reply with speaker number, else reply with 0',
                            parse_mode='html')
